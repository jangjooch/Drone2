    ...
    public void cameraConnect(int cameraNo, int width, int height, double angle) {
        ...
        while (true) {
            try {
                ...
                // openCV 라이브러리 등록
                // -Djava.library.path=/usr/lib/jni:/home/pi/opencv/opencv-3.4.5/build/lib
                // 이것이 프로그램 시작에 있어 외부 라이브러리를 가져와 사용할 수 있도록 지정하는 것이다.
                // openCV 라이브러리는 Raspi 자체에 설치 되어있는 것을 가져와야 하기 때문이다.
                // /usr/lib/jni 은 C 기반의 openCV를 자바에서 사용하기 위한 라이브러리가 있는 곳이다.
